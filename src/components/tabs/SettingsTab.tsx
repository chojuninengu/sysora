...
export function SettingsTab() {
  ...
  if (!settings) {
    return (
      <div className="p-6 text-white/40 flex items-center gap-2">
  ...
  return (
    <div className="p-6 pb-20 space-y-8 max-w-2xl mx-auto overflow-y-auto max-h-[85vh]" style={{ scrollbarWidth: "thin" }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/40 font-medium">Configure how Sysora monitors and behaves</p>
          </div>
        </div>
      ...
      <section className="space-y-4">
        ...
        <div className="bg-white/[0.03] rounded-2xl border border-white/10 p-5 space-y-6">
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <label className="text-sm text-white/90 font-semibold">Refresh Interval</label>
              <span className="text-xs font-mono font-bold text-brand-400 bg-brand-400/10 px-2 py-0.5 rounded border border-brand-400/20">
      ...
      <section className="space-y-4">
        ...
        <div className="bg-white/[0.03] rounded-2xl border border-white/10 p-5 space-y-8">
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <label className="text-sm text-white/90 font-semibold">CPU Alert</label>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              <label className="text-sm text-white/90 font-semibold">RAM Alert</label>
              <span className="text-xs font-mono font-bold text-brand-400 bg-brand-400/10 px-2 py-0.5 rounded border border-brand-400/20">
      ...
      <div className="pt-4 flex justify-center">
        <button
          ...
          className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-white/20 hover:text-red-400 transition-all bg-white/[0.02] hover:bg-red-400/10 px-5 py-2.5 rounded-xl border border-white/5 hover:border-red-400/20"
        >
          <RotateCcw size={12} />
          Reset to Defaults
        </button>
...
function Toggle({
  ...
  return (
    <div className="p-5 flex items-center justify-between hover:bg-white/[0.02] transition-all group">
      <div>
        <h4 className="text-sm font-semibold text-white group-hover:text-brand-400 transition-colors">{label}</h4>
        <p className="text-[10px] text-white/40 font-medium">{description}</p>
      </div>
      <button
        onClick={() => onChange(!value)}
        className={`w-10 h-5 rounded-full relative transition-all duration-300 ${
          value ? "bg-brand-600 shadow-[0_0_10px_rgba(83,74,183,0.4)]" : "bg-white/10"
        }`}
      >
        <div
